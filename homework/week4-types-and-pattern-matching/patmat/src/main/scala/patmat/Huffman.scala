...
//  http://stackoverflow.com/questions/22966705/scala-what-is-the-difference-between-size-and-length-of-a-seq
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    ...
    if (trees.length <= 2) trees
      ...
      val left = trees(0)
      val right = trees(1)
      val fork = Fork(left, right, chars(left) ::: chars(right),
        weight(left) + weight(right))
      combine((fork :: trees.drop(1).drop(1)).sortBy(weight))
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def decodeOneChar(subtree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = subtree match {
      ...
      case Fork(left, right, chars, weight) => {
        val b = bits.head
        if (b == 0) decodeOneChar(left, bits.tail)
        else decodeOneChar(right, bits.tail)
    ...
    while (remainingBits.nonEmpty) {
      var p = decodeOneChar(tree, remainingBits)
      res = res ::: List(p._1)
      remainingBits = p._2
      // print(remainingBits.length)
    }
